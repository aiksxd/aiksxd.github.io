...
function refreshMap(fnOfEcho){
    document.getElementById("roomTitle").textContent = nodesMap[4];
    ...
    if(lastAimId === aimId){  // if second click on same button
        switch (fnOfEcho) {
            ...
            case 1:     // button for joining a node
                ...
                if (parent_Node) {
                    document.location.href = "./audience.html?id=" + aimId +"&name="+ getMyName() + "&themeIndex="+ config['theme_Index'];
                } else {
                    if (config['app_Mode']) {
                        window.parent.postMessage(['jump', app_Info['app_Name'], app_Info['app_Id'], "apps/PeerLive/audience.html?id=" + aimId +"&name=&themeIndex="+ config['theme_Index']]);
                    } else {
                        window.open("./audience.html?id=" + aimId +"&name=&themeIndex="+ config['theme_Index']);
                    }
                }
                break;
            case 2:     // button for joining a node
                if(aimId == peer.id){alert(aimId);break;}
                if (parent_Node) {
                    document.location.href = "./P2PGameFiveOnLinePlayer.html?id=" + aimId +"&name="+ getMyName() + "&themeIndex="+ config['theme_Index'];
                } else {
                    if (config['app_Mode']) {
                        window.parent.postMessage(['jump', app_Info['app_Name'], app_Info['app_Id'], "apps/PeerLive/P2PGameFiveOnLinePlayer.html?id=" + aimId +"&name=&themeIndex="+ config['theme_Index']]);
                    } else {
                        window.open("./P2PGameFiveOnLinePlayer.html?id=" + aimId +"&name=&themeIndex="+ config['theme_Index']);
                    }
                }
                break;
        }
...
function sendMsg() {   //*This line of code adds an event listener to the element with the ID "sendButton". It listens for a click event on the button and triggers the provided function when the event occurs.
    ...
    if (msg[3] || msg[4]){    // DEBUG *This condition checks if the second element of the "msg" array (i.e., the message content) exists and is not empty.
        ...
        document.getElementById("sendMessageBox").setAttribute("placeholder", res_Strings['fn'][14][config['language']]);
...
function sendImg(){
    ...
    if(file){
        if(file.size <= 1048576){
            ...
            alert(res_Strings['fn'][15][config['language']] + " 1MB");
        }
    } else {
...
function upload_Conferee_Video() {
    if (conference_Stream) {
        if (my_Conferee_Index) {
            ...
            alert(res_Strings['fn'][16][config['language']]);
        }
    }
}
...
function update_Conferee_Info() {
    if (my_Conferee_Index) {
        ...
        conference_Send(msg);
        document.getElementById('conferees'+ my_Conferee_Index).getElementsByClassName('confereeName')[0].textContent = msg[5];
    }
}
...
function join_Conference() {
    ...
    if (document.getElementById('joinConference').checked) {
        ...
        if (!nodesMap[11]) {
            alert(res_Strings['fn'][17][config['language']]);
            document.getElementById('joinConference').checked = false;
            return;
        }
...
if (document.getElementById('copyURL')) {
    document.getElementById('copyURL').addEventListener('click', function() {
        ...
        if (parent_Node) {
            url = url +"audience.html?id="+ parent_Node.peer +"&name=";
        } else {  // host or audience partly unloaded
            url = url +"audience.html?id="+ peer.id +"&name=";
        }
        
        navigator.clipboard.writeText(url)
            .then(() => {
                alert(res_Strings['fn'][18][config['language']] + currentUrl);
            })
            .catch(err => {
...
window.addEventListener('message', function(event) {
    ...
    // let urlIndex = currentUrl.indexOf(config['language']);
    // if (urlIndex !== -1) {
